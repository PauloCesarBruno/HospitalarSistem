...
{
    public partial class frmConsulta_Exames : Form
    {
        public String codigo { get; set; }

        public frmConsulta_Exames()
        {
            InitializeComponent();
            dgvConsultaExame.AutoGenerateColumns = false;
        }
        private void CarregarGrid() // Carrega e Consulta por Nome.
        {
            try
            {
                ...
                dgvConsultaExame.DataSource = null; // dgv Escondido Atras do Picturebox
                dgvConsultaExame.DataSource = Eedcolecao;
                dgvConsultaExame.Update();
                dgvConsultaExame.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void frmConsulta_Exames_Load(object sender, EventArgs e)
        {
            CarregarGrid();
       ...
       .DisableCloseButton(this.Handle.ToInt32()); // Desabilitar o "X".           
        ...
        private void txtBuscaNome_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                CarregarGrid();
                if (dgvConsultaExame.Rows.Count == 0)
                {
                ...
                txtCapCodigo.Text = dgvConsultaExame.CurrentRow.Cells[0].Value.ToString();                
            }
            catch (Exception)
            {
                //
        ...
        private void frmConsulta_Exames_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void dgvConsultaExame_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow row = this.dgvConsultaExame.Rows[e.RowIndex];
