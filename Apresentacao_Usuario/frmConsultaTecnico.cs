...
namespace Apresentacao_Usuario
{
    public partial class frmConsultaTecnico : Form
    {
        ...
        {
            InitializeComponent();
            dgvConsultaTecnico.AutoGenerateColumns = false;
        }
        ...
        private void CarregarGrid() // Carrega e Consulta por Nome.
        {
            try
            {
                ...
                NegociosTecnicosEnfermagem NTenfermagem = new NegociosTecnicosEnfermagem();

                Tecolecao = NTenfermagem.ConsultaTecnicoNome(txtBuscaNome.Text);
                dgvConsultaTecnico.DataSource = null;
                dgvConsultaTecnico.DataSource = Tecolecao;
                dgvConsultaTecnico.Update();
                dgvConsultaTecnico.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void ConsultaId()
        {
            try
            {
                ...
                Tecolecao = NTenfermagem.ConsultaTecnicoId(Convert.ToInt32(txtBuscaId.Text));
                dgvConsultaTecnico.DataSource = null;
                dgvConsultaTecnico.DataSource = Tecolecao;
                dgvConsultaTecnico.Update();
                dgvConsultaTecnico.Refresh();
        ...
        {
            CarregarGrid();
            if (dgvConsultaTecnico.Rows.Count == 0)
            {
        ...
        private void frmConsultaTecnico_Load(object sender, EventArgs e)
        {
            CarregarGrid();
      ...
      .DisableCloseButton(this.Handle.ToInt32()); // Desabilitar o "X".           
        ...
        private void frmConsultaTecnico_KeyDown(object sender, KeyEventArgs e)
        {
