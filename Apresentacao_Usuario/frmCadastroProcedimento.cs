...
{
    public partial class frmCadastroProcedimento : Form
    {
        public frmCadastroProcedimento()
        {
            InitializeComponent();
        }
        ...
        private void CarregarGrid() // Carrega e Consulta por Nome.
        {
            try
            {
                ProcedimentoColecao Proccolecao = new ProcedimentoColecao();
                NegociosProcedimento Nproc = new NegociosProcedimento();

                Proccolecao = Nproc.ConsultaProcedimentoNome(txtBuscaNome.Text);
                dgvCadProced.DataSource = null; // dgv Escondido Atras do Picturebox
                dgvCadProced.DataSource = Proccolecao;
                dgvCadProced.Update();
                dgvCadProced.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void ConsultaId()
        {
            try
            {
                ...
                Proccolecao = Nproc.ConsultaProcedimentoId (Convert.ToInt32 (txtBuscaId.Text));
                dgvCadProced.DataSource = null; // dgv Escondido Atras do Picturebox
                dgvCadProced.DataSource = Proccolecao;
                dgvCadProced.Update();
                dgvCadProced.Refresh();
        ...
        private void Inserir()
        {
            try
            {
                ...
                if (txtCodigo.ReadOnly == false && txtNome.ReadOnly == false)
                {

                    Procedimento procedimento = new Procedimento();
                    NegociosProcedimento Nproc = new NegociosProcedimento();

                    procedimento.IdProcedimento = Convert.ToInt32(txtCodigo.Text);
                    procedimento.ProcedimentoInternacao = txtNome.Text;
                    dgvCadProced.DataSource = Nproc.InsereProcedimento(procedimento);
                }
                else
                {
                    //
        ...
        private void Alterar()
        {
            try
            {
                ...
                if (txtCodigo.ReadOnly == false && txtNome.ReadOnly == false)
                {
                    ...
                    procedimento.IdProcedimento = Convert.ToInt32(txtCodigo.Text);
                    procedimento.ProcedimentoInternacao = txtNome.Text;
                    dgvCadProced.DataSource = Nproc.AlteraProcedimento(procedimento);
                }
                else
                {
                    //
        ...
        private void Deletar()
        {
            try
            {
                ...
                if (txtCodigo.ReadOnly == false && txtNome.ReadOnly == false)
                {
                    ...
                    procedimento.IdProcedimento = Convert.ToInt32(txtCodigo.Text);
                    dgvCadProced.DataSource = Nproc.ExcluiProcedimento(procedimento);
                }
                else
                {
                    //
        ...
        private void CarregarGridVer() // Carrega e Consulta por Nome.
        {
            try
            {
                AmarraExclusaoProcedimentoColecao Procolecao = new AmarraExclusaoProcedimentoColecao();
                NegociosAmarracaoProcedimentos Amarra = new NegociosAmarracaoProcedimentos();
        ...
        private void frmCadastroProcedimento_Load(object sender, EventArgs e)
        {
            CarregarGrid();
            CarregarGridVer();
      ...
      .DisableCloseButton(this.Handle.ToInt32()); // Desabilitar o "X".           
        ...
        private void dgvCadProced_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow row = this.dgvCadProced.Rows[e.RowIndex];
        ...
        private void frmCadastroProcedimento_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnInserir_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Para Adicionar um Novo Procedimento, clicar Prieiramente no Botão (Novo Procedimento)", "SISTEMA HOSPITALAR", MessageBoxButtons.OK, MessageBoxIcon.Information);
                btnNovo.Focus();
                return;
            }
            if (txtCodigo.Text == String.Empty || txtNome.Text == String.Empty)
            {
                MessageBox.Show("Para Inserção de Novo Procedimento é Necessário Preencher todos os campos.", "SISTEMA HOSPITALAR", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Limpa_Campos();
                txtCodigo.Focus();
                return;
            }
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            ...
            DialogResult perg = MessageBox.Show("Deseja Relamente Excluir o Registro deste Procedimento) ?", "SISTEMA HOSPITALAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            {
                MessageBox.Show("Não é Possivel Excluir Este Procedimento, por motivo de que existe Paciente(s) vinculado ao Procedimento que vc está tentando excluir.", "SISTEMA HOSPITALAR", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Limpa_Campos();
                txtBuscaNome.Focus();
                return;
            }
