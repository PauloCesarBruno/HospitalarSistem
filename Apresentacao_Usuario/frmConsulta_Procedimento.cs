...
{
    public partial class frmConsulta_Procedimento : Form
    {
        public String codigo { get; set; }

        public frmConsulta_Procedimento()
        {
            InitializeComponent();
            dgvConsultaProced.AutoGenerateColumns = false;
        }
        private void CarregarGrid() // Carrega e Consulta por Nome.
        {
            try
            {
                ...
                dgvConsultaProced.DataSource = null; // dgv Escondido Atras do Picturebox
                dgvConsultaProced.DataSource = Proccolecao;
                dgvConsultaProced.Update();
                dgvConsultaProced.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void frmConsulta_Procedimento_Load(object sender, EventArgs e)
        {
            CarregarGrid();
        ...
        private void txtBuscaNome_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                CarregarGrid();
                if (dgvConsultaProced.Rows.Count == 0)
                {
                ...
                txtCapCodigo.Text = dgvConsultaProced.CurrentRow.Cells[0].Value.ToString();               
            }
            catch (Exception)
            {
                //
        ...
        private void frmConsulta_Procedimento_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void dgvConsultaProced_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow row = this.dgvConsultaProced.Rows[e.RowIndex];
