...
namespace Apresentacao_Usuario
{
    public partial class frmConsultaPaciente : Form
    {
        ...
        {
            InitializeComponent();
            dgvConsultaPacientes.AutoGenerateColumns = false;
        }
        ...
        private void CarregarGrid() // Carrega e Consulta por Nome.
        {
            try
            {
                ConsultaPacienteColecao CCcolecao = new ConsultaPacienteColecao();
                NegociosConsultaPaciente NCpaciente = new NegociosConsultaPaciente();

                CCcolecao = NCpaciente.ConsultaPacientePorNome(txtBuscaNome.Text);
                dgvConsultaPacientes.DataSource = null;
                dgvConsultaPacientes.DataSource = CCcolecao;
                dgvConsultaPacientes.Update();
                dgvConsultaPacientes.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void ConsultaId()
        {
            try
            {
                ...
                CCcolecao = NCpaciente.ConsultaPacientePorId(Convert.ToInt64(txtBuscaId.Text));
      ...
      .DisableCloseButton(this.Handle.ToInt32()); // Desabilitar o "X".           
        ...
        {
            CarregarGrid();
            if (dgvConsultaPacientes.Rows.Count == 0)
            {
        ...
        private void frmConsultaPaciente_KeyDown(object sender, KeyEventArgs e)
        {
